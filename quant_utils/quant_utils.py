...
def linear_quantize(input, scale, zero_point=None, inplace=False):
    """
...
def linear_dequantize(input, scale, zero_point=None, inplace=False):
    """
...
# TODO Symmetric linear quantization implementation
...
def symmetric_linear_quantization_params(num_bits,
                                          ...
                                          signed=True):
    """
    Compute the scaling factor with the given quantization range.
    saturation_alpha: symmetric bound for quantization range
    """
    n = 2**num_bits - 1
    scale = n / torch.clamp((2 * alpha), min=1e-8)    
    return scale
...
class SymmetricQuantFunction(Function):
    ...
    """
    @staticmethod
    def forward(ctx, x, k, x_alpha=None):
        """
        x: single-precision value to be quantized
        k: bit-setting for x
        x_min: lower bound for quantization range
        ...
        if x_alpha is None:
            x_min, x_max = x.min(), x.max()
            x_alpha = max(abs(x_min), abs(x_max))
        scale = symmetric_linear_quantization_params(k, x_alpha)
        new_quant_x = linear_quantize(x, scale, zero_point=None, inplace=False)
        n = 2**(k - 1)
        new_quant_x = torch.clamp(new_quant_x, -n, n - 1)
        quant_x = linear_dequantize(new_quant_x,
                                    scale,
